    ...
    return (
        <div className="animate-fade-in space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <Card>
                    ...
                    <CardContent className="h-64 flex items-center justify-center">
                        {isLoading ? <div className="w-48 h-48 rounded-full bg-slate-100 dark:bg-slate-800 animate-pulse"></div> : <DoughnutChart data={deviceData} />}
                    </CardContent>
                </Card>
                <Card>
                    <CardHeader>
                    ...
                    <CardContent className="h-64 flex items-center justify-center">
                        {isLoading ? <div className="w-48 h-48 rounded-full bg-slate-100 dark:bg-slate-800 animate-pulse"></div> : <DoughnutChart data={genderData} />}
                    </CardContent>
                </Card>
                <Card>
                    <CardHeader>
                    ...
                    <CardContent>
                        {isLoading ? (
                            <div className="space-y-4">
                                {[1, 2, 3, 4, 5].map(i => <div key={i} className="h-8 w-full bg-slate-100 dark:bg-slate-800 rounded animate-pulse"></div>)}
                            </div>
                        ) : (
                            <div className="space-y-4">
                                {[
            ...
            <Card>
                ...
                <CardContent>
                    {isLoading ? (
                        <div className="h-48 w-full bg-slate-100 dark:bg-slate-800 rounded-xl animate-pulse"></div>
                    ) : (
